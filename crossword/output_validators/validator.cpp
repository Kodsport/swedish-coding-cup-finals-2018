...
static const char wordlistStr[] = R"(
aardvark
aardvarks
abaci
aback
...
bool connected(vector<string>& grid, int N, int M) {
	int si = -1, sj = -1;
	rep(i,0,N) rep(j,0,M) {
		if (grid[i][j] != '.') {
			si = i, sj = j;
	...
	if (si == -1) return true; // we treat empty grids as connected
	...
	rep(i,0,N) rep(j,0,M) {
		if (grid[i][j] != '.' && !seen[i][j]) return false;
	}
	return true;
}
...
int main(int argc, char** argv) {
	...
	{
		string temp;
		for (char c : wordlistStr) {
			if (!c) {}
			else if (c == '\n') {
				if (!temp.empty()) wordlistSet.insert(temp);
				temp.clear();
			} else if ('a' <= c && c <= 'z') {
				temp += c;
			} else {
	...
	auto validateWord = [&](string& w) {
		if (w.size() > 1 && !wordlistSet.count(w))
			die("not a word: " + w);
		w = "";
	};
	...
	auto solve = [&](istream& is, int N, int M) -> pair<int, bool> {
		...
		rep(i,0,N) {
			...
			rep(j,0,M) {
				char c = line[j], orig = ingrid[i][j];
				if ('a' <= orig && orig <= 'z') {
					if (c != orig) die("overwrite");
				} else {
					assert(orig == '.');
					if (!('a' <= c && c <= 'z') && c != '.') die("invalid character " + to_string(c));
				}
				if (c != '.') res++;
			}
			grid[i] = line;
		...
		rep(it,0,2) {
			...
			rep(i,0,N) rep(j,0,M) grid2[j][i] = grid[i][j];
			grid = grid2;
			swap(N, M);
		}
